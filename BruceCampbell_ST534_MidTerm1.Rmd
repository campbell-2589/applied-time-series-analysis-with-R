...
Without loss of generality we can assume that $E[S_t]=0$ and $E[N_t]=0$ otherwise we'll subtract $\mu_S$ and $\mu_N$ from $S_t$ and $N_t$ and $\mu_S+ A \mu_S + \mu_N$ from $X_t$.  Now that we've mean corrected, we can write all of the ACvF functions in the redcued form.
...
Colecting terms and using a trig identity, we have that.


$$f_X(\omega) = [1 +A^2 +  A \; cos(2 \pi i \omega D) ] \; f_S(\omega) \; + f_N(\omega)$$
Uniqueness of the Fourier transform assures us that the expression on the RHS is indeed the spectras density of $X$.
